...
class ZipcodePage {
    ...
    get VerifyState () {
        return $('//div[@class="location-card  active"]/p')
    }
    ...
    async executeZipcodeTest () {
        for (let i = 0; i < ZipCodeArray.length; i++){
            ...
            const stateText = await this.VerifyState.getText();
            const lastTwoCharacters = stateText.slice(-2);
            console.log('Last two characters:', lastTwoCharacters);
            
            // await expect(lastTwoCharacters).toHaveText(StateArray[i]);
        }
    }
